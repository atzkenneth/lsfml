...
def get_predictions(smi, ern, config_id, fname):
    ...
    model1.load_state_dict(torch.load(f"{model_path}config_{config_id}_1.pt",map_location=torch.device("cpu")))
    model1 = model1.to(DEVICE)
    model2 = GraphTransformer(N_KERNELS, POOLING_HEADS, D_MLP, D_KERNEL, D_EMBEDDING, QML, GEOMETRY)
    model2.load_state_dict(torch.load(f"{model_path}config_{config_id}_2.pt",map_location=torch.device("cpu")))
    model2 = model2.to(DEVICE)
    model3 = GraphTransformer(N_KERNELS, POOLING_HEADS, D_MLP, D_KERNEL, D_EMBEDDING, QML, GEOMETRY)
    model3.load_state_dict(torch.load(f"{model_path}config_{config_id}_3.pt",map_location=torch.device("cpu")))
    ...
    for idx, sm in enumerate(tqdm(smi)):
        ...
        try: 
            ...
            for acid in ACIDS:
                ...
                prd = int(np.mean(np.array([pred1, pred2, pred3])) * 100)
                std = int(np.std(np.array([pred1, pred2, pred3])) * 100)
    ...
    out_name = f"labelled_data/labels_{config_id}_{fname}.xlsx"
    os.makedirs(os.path.dirname(out_name), exist_ok=True)
    df.to_csv(f"labelled_data/labels_{config_id}_{fname}.csv", index=False,)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-csv", type=str, default="Minisci_Substrates")
    args = parser.parse_args()
    
    # Read csv/xlsx
    fname = args.csv
    try:
        data = pd.read_excel(f"../data/{fname}.xlsx", engine='openpyxl')
    except:
    ...
    get_predictions(smi, ern, config_id=args.config, fname=fname)
