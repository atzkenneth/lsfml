...
class GraphTransformer(nn.Module):
    def __init__(self, n_kernels=3, pooling_heads=8, mlp_dim=512, kernel_dim=128, embeddings_dim=128, out_dim=1, qml=True, geometry=True):
        ...
        self.atom_em = nn.Embedding(
            num_embeddings=10, embedding_dim=self.embeddings_dim
        )
        self.ring_em = nn.Embedding(
            num_embeddings=2, embedding_dim=self.embeddings_dim
        )
        self.hybr_em = nn.Embedding(
            num_embeddings=4, embedding_dim=self.embeddings_dim
        )
        self.arom_em = nn.Embedding(
            num_embeddings=2, embedding_dim=self.embeddings_dim
        )
        ...
        for _ in range(self.n_kernels):
            if self.geometry == True:
                self.kernels.append(
                    EGNN_sparse3D(
                        feats_dim=self.kernel_dim, m_dim=self.m_dim, aggr=self.aggr,
                    )
                )
        ...
        if self.qml == True:
            post_egnn_mlp_inp_dim = self.kernel_dim * (self.n_kernels + 2)
        elif self.qml == False:
        ...
        if self.qml == True:
            post_egnn_mlp2_inp_dim = self.kernel_dim * (self.n_kernels + 2)
        elif self.qml == False:
        ...
        self.con_lin = nn.Linear(3, self.embeddings_dim)

        self.post_pooling_mlp_input_dim = self.kernel_dim * (self.pooling_heads + 1) + self.embeddings_dim
    ...
    def forward(self, g_batch, g_batch2):
        ...
        # General embedding
        if self.qml == True:
            qml_features1 = self.chrg_em(g_batch.charges)
            ...
            qml_features2 = self.chrg_em(g_batch2.charges)
        ...
        if self.qml == True:
            feature_list.append(qml_features1)
        elif self.qml == False:
            pass
        ...
        if self.qml == True:
            feature_list2.append(qml_features2)
        elif self.qml == False:
            pass
        ...
        glob_features_2 = self.transformer2(
            x=features, batch=g_batch2.batch, edge_index=g_batch2.edge_index
        )
            
        # Concatenating conditions and finaling post pooling mlp
        conditions = self.con_lin(g_batch.condtns)
        features = torch.cat([glob_features_1, glob_features_2, conditions], dim=1)
        # features = self.post_pooling_mlp(features).squeeze(1)

        layers = [module for module in self.post_pooling_mlp.modules() if not isinstance(module, nn.Sequential)]

        for layer in layers[:-1]:
            features = layer(features)
        
        # output = layers[-1](features).squeeze(1)
...
class EGNN_sparse3D(MessagePassing):
    def __init__(
        ...
        **kwargs
        ...
        self.edge_input_dim = (
            (self.fourier_features * 2) + 1 + (feats_dim * 2)
        )
...
if __name__ == "__main__":
    ...
    # model
    model = GraphTransformer(n_kernels=3, pooling_heads=4, mlp_dim=256, kernel_dim=64, embeddings_dim=64, qml=True, geometry=False)
