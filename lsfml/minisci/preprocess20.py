...
if __name__ == "__main__":
    name = "20230608_alkylation_rxndata_full_all_new"
    df = pd.read_csv(f"data/{name}.csv", sep=",", encoding="unicode_escape")  # *hte.h5

    # read csv id
    rxn_id = list(df["rxn_id"])
    temperature_degC = list(df["temperature_degC"])
    time_h = list(df["time_h"])
    atmosphere = list(df["atmosphere"])  # ID
    atmosphere = [str(x) for x in atmosphere]
    scale_mol = list(df["scale_umol"])
    concentration_moll = list(df["concentration_mmoll"])
