...
```
Install a suitable version of Pytorch and Pytorch Geometric. The herein described software was tested on PyTorch Geometric (2.0.2) and PyTorch (1.10.1+cu102). Additional information about the installation of Pytorch Geometric can be found here: https://pytorch-geometric.readthedocs.io/en/latest/notes/installation.html
```
...
`modules/`contains additional essential neural network modules used by the different graph neural networks. `gnn_blocks.py` contains the message passing functions for 2D and 3D graphs that are used by all GNNs and GTNNs, and `gmt.py` contains the graph multiset transformer-based pooling that is used by all GTNNs. 
```
ls modules/
gmt.py  gnn_blocks.py
```

## 10. Citation (Preprint)
```
@article{nippa2022enabling,
  ...
  journal={ChemRxiv preprint 10.26434/chemrxiv-2022-gkxm6},
  year={2022}
}
```
